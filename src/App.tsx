...
import {
  ...
  Video, VideoOff, TrendingUp, Camera
} from 'lucide-react';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'train' | 'library' | 'history' | 'dashboard'>('train');
  ...
  useEffect(() => {
    // During recording, speech-tracking drives the scroll instead
    if (!isPlaying || !teleprompterContainerRef.current || isRecording) {
  ...
  const handleStartRecording = async () => {
    try {
      ...
      if (videoMode && videoPreviewStream) {
      ...
      <div className="min-h-screen bg-[#050507] text-[#e0e0e0] font-sans flex flex-col">
        <nav className="h-16 flex items-center justify-between px-6 md:px-12 bg-[#0a0a0e]/85 backdrop-blur-md border-b border-white/5 z-40 sticky top-0">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-sm text-white">VOCALISE <span className="text-amber-400">ADMIN</span></span>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
              ...
              <span className="text-[10px] font-mono text-white/60 max-w-[120px] truncate">{user.name}</span>
            </div>
            <button
              onClick={logout}
                  ...
                  <div className="pt-4 flex flex-col gap-2">
                    ...
                    {recordedVideoUrl && (
                      <a
                        ...
                        download={`vocalise_video_${Date.now()}.webm`}
